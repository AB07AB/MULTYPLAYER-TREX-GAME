...
var starGroup, GoldStarIMG;

var bronzeGroup, BronzeStarIMG;
var crownIMG;
...
//-------------------STARS------------------
GoldStarIMG = loadImage("download-1.jpg");

BronzeStarIMG = loadImage("download.jpg")
crownIMG = loadImage("Crown.jpg");
...
trex2 = createSprite(50,660,20,50);
  ...
  crown = createSprite(200,42,20,50)
  crown.addImage("crown", crownIMG)
  crown.scale = 0.05;
  crown.visible = false;

  crown1 = createSprite(200,515,20,50)
  crown1.addImage("crown", crownIMG)
  crown1.scale = 0.05;
...
function draw() {
  ...
  if (gameState===PLAY){
    ...
    //crown.velocityY = crown.velocityY + 0.8

    trex2.velocityY = trex2.velocityY + 0.8
    //crown1.velocityY = crown1.velocityY + 0.8
    ...
    //crown.collide(invisibleGround);
    trex2.collide(invisibleGround2);
    //crown1.collide(invisibleGround2);
    spawnClouds();
    spawnObstacles();
    //-----STARS------
    spawnStars();
  ...
  if(bronzeGroup.isTouching(trex)){
    crown.visible=true;
   }

   if(bronzeGroup.isTouching(trex2)){
    crown1.visible=true;
   }
  ...
  if(starGroup.isTouching(trex)){
    ...
    textSize(20)
    textFont("BOLD")
    text("YOU WON THE SUPER GOLD STAR", 500,100)
    gameOver.visible=false
    
    }

  if(starGroup.isTouching(trex2)){
      ...
      textSize(20)
      textFont("Georgia")
      text("YOU WON THE SUPER GOLD STAR", 300,570)
      gameOver.visible=false
...
function bronzeStars(){
  if (frameCount % 500 === 0){
    var bronze = createSprite(600,80,40,10);
    var bronze1 = createSprite(600,500,40,10);

    bronze.y = Math.round(random(80,120));
    bronze1.y = Math.round(random(550,600));

    bronze.addImage(BronzeStarIMG);
    bronze1.addImage(BronzeStarIMG);
    ...
    bronze.lifetime = 500;
    bronze1.lifetime = 500;

    bronze.depth = trex.depth;
    trex.depth = trex.depth + 1;

    bronze1.depth = trex.depth;
    trex.depth = trex.depth + 1;

    bronzeGroup.add(bronze);
    bronzeGroup.add(bronze1);

    if(bronzeGroup.isTouching(trex)){
      bronzeGroup.visible=false
    }
    if(bronzeGroup.isTouching(trex2)){
      bronzeGroup.visible=false
    }
...
function spawnStars() {
  ...
  if (frameCount % 1000 === 0) {
    ...
    var star1 = createSprite(600,500,40,10);

    star.y = Math.round(random(20,140));
    star1.y = Math.round(random(20,140));

    star.addImage(GoldStarIMG);
    star1.addImage(GoldStarIMG);
